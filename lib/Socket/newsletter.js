...
const makeNewsletterSocket = (config) => {
    ...
    const parseFetchedUpdates = async (node, type) => {
        ...
        return await Promise.all((0, WABinary_1.getAllBinaryNodeChildren)(child).map(async (messageNode) => {
            ...
            .map(({
                ...
                count: +attrs.count, code: attrs.code
            }));
            const data = {
    ...
    return {
        ...
        subscribeNewsletterUpdates: async (jid) => {
            ...
            return (_a = (0,
                WABinary_1.getBinaryNodeChild)(result,
                'live_updates')) === null || _a === void 0 ? void 0: _a.attrs;
        },
        ...
        newsletterUpdateDescription: async (jid, description) => {
            await newsletterWMexQuery(jid, QueryIds.JOB_MUTATION, {
                updates: {
                    description: description || '', settings: null
                }
            });
        },
        ...
        newsletterUpdatePicture: async (jid, content) => {
            ...
            await newsletterWMexQuery(jid, QueryIds.JOB_MUTATION, {
                updates: {
                    picture: img.toString('base64'), settings: null
                }
            });
        },
        ...
        newsletterFetchAllParticipating: async () => {
            ...
            let newsletterMetadata = async (type, key, role) => {
                const result = await newsletterWMexQuery(undefined, QueryIds.METADATA, {
                    input: {
                        key,
                        type: type.toUpperCase(),
            ...
            try {
                ...
                for (const i of newsletters) {
                    if (!WABinary_1.isJidNewsletter(i.id)) continue;

                    try {
                        const metadata = await newsletterMetadata('JID', i.id);
                        if (metadata) data[metadata.id] = metadata;
                    } catch (err) {
                        console.warn(`Skip newsletter ${i.id} karena error:`, err.message);
                        continue; // skip newsletter yang error
                    }
                }
            } catch (err) {
                console.error('Gagal fetch newsletters:', err.message);
            }
        ...
        newsletterReactMessage: async (jid, server_id, code) => {
            await query( {
                ...
                attrs: {
                    to: jid, ...(!code ? {
                        edit: '7'
                    }: {}), type: 'reaction', server_id, id: (0, Utils_1.generateMessageID)()
                },
                content: [{
                    tag: 'reaction',
                    attrs: code ? {
        ...
        newsletterFetchMessages: async (type, key, server_id, after) => {
            const afterStr = after?.toString();

            // fetch semua pesan (count default 100)
            ...
            // parse semua pesan
            const messages = await parseFetchedUpdates(result, 'messages');

            // filter berdasarkan server_id
            return messages.filter(msg => msg.server_id?.toString() === server_id.toString());
        },
        newsletterFetchUpdates: async (jid, count, after, since) => {
            const result = await newsletterQuery(jid, 'get', [{
                tag: 'message_updates',
                attrs: {
                    count: count.toString(), after: (after === null || after === void 0 ? void 0: after.toString()) || '100', since: (since === null || since === void 0 ? void 0: since.toString()) || '0'
                }
            }]);
